...
* MiniCypher reads and writes including `MATCH`, `CREATE`, `MERGE`, scalar and map-based `SET`, `REMOVE`, `DELETE`, `DETACH DELETE`, `WITH`, `UNWIND`, `OPTIONAL MATCH`, parameters, aggregates, `ORDER BY`, `SKIP`, and `LIMIT`.
...
The project is not compatible with full Neo4j/Cypher. Major remaining areas include richer map expressions, general procedures, path values, subqueries, weighted path algorithms, community detection, similarity, embeddings, flow algorithms, fuzzing, CI, and large-scale performance work.
